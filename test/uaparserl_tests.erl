    ...
    [
     ...
     {"normalize_pattern/1 returns expected output",
      fun () ->
              ?assertEqual(
                 {test_atom, "string"},
                 uaparserl:normalize_pattern({"test_atom", "string"})
                )
      end
     },
     {"normalize_pattern_sets/1 returns expected output]",
      fun () ->
              ?assertMatch(
                 ...
                 uaparserl:normalize_pattern_sets(
                   ?EXAMPLE_UA
                  )
                )
      end
     ...
     {"get_regex_flags/1 returns empty list when regex_flag is missing",
      fun () ->
              ?assertEqual(
                 [],
                 uaparserl:get_regex_flags([])
                )
      end
     },
     {"compile_pattern_set/1 successfully compiles with flag",
      fun () ->
              Result = uaparserl:compile_pattern_set(
                         [
                          {regex, "(WebTV)/(\d+).(\d+)"},
                          {regex_flag, "i"}
                         ]
...
private_parse_funs_test_() ->
    [
     {"format_result/4 formats text with placeholders correctly",
      fun () ->
              _UserAgentString = "Mozilla/5.0 (Windows; Windows NT 5.1; rv:2.0b3pre) Gecko/20100727 Minefield/4.0.1pre",
              ?assertEqual(
                 {useragent, [{family, "Firefox (Minefield)"},
                              {major, "4"},
                              {minor, "0"},
                              {patch, "1pre"}]},
                 uaparserl:format_result(
                   useragent,
                   ["Minefield", "4", "0", "1pre"],
     ...
     {"format_result/4 formats multiple placeholders correctly",
      fun () ->
              _UserAgentString = "Mozilla/5.0 (Linux; U; Android 4.2.2; de-de; PEDI_PLUS_W Build/JDQ39) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Safari/534.30",
              ?assertEqual(
                 {device, [{family, "Odys PEDI PLUS W"},
                           {brand, "Odys"},
                           {model, "PEDI PLUS W"}]},
                 uaparserl:format_result(
                   device,
     ...
     {"parse/1 parses device placeholders correctly",
      fun () ->
              UserAgentString = "Mozilla/5.0 (Linux; Android 4.2.2; ZP998 Special Build/JDQ39) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/33.0.1750.170 Mobile Safari/537.36",
              ?assertEqual(
                 [{useragent,
...
device_cases_test_() ->
    {setup,
     fun load_device_cases/0,
     fun test_device_cases/1
    }.
